...
function PallyPower:OnEnable()
	...
	-- Kronos: free assignment defaults to ON (one-time flip so existing
	-- saved profiles pick it up too)
	if not self.opt.kronosFreeDefaultApplied then
		self.opt.kronosFreeDefaultApplied = true
		self.opt.freeassign = true
	end
	self:ScanSpells()
	self:ScanCooldowns()
...
function PallyPower:OpenConfigWindow()
	...
	-- Kronos: simple options window with the same option set as the 112
	-- build, replacing the full Ace3 config dialog
	if not self.SimpleOptions then
		local f = CreateFrame("Frame", "PallyPowerSimpleOptions", UIParent, "BackdropTemplate")
		f:SetSize(300, 220)
		...
		f:Hide()
		table.insert(UISpecialFrames, "PallyPowerSimpleOptions")

		local title = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
		title:SetPoint("TOP", f, "TOP", 0, -10)
		title:SetText("Pally Power - Options")
		...
		local function Check(name, label, y, get, set)
			local cb = CreateFrame("CheckButton", name, f, "UICheckButtonTemplate")
			cb:SetSize(26, 26)
			cb:SetPoint("TOPLEFT", f, "TOPLEFT", 14, y)
			local fs = _G[name .. "Text"] or cb.Text
			if fs then
				fs:SetText(label)
			end
			cb:SetScript("OnShow", function(s) s:SetChecked(get()) end)
			cb:SetScript("OnClick", function(s) set(s:GetChecked() and true or false) end)
			return cb
		end

		Check("PallyPowerSimpleOptLock", "Lock the buff bar", -36,
			function() return PallyPower.opt.display.frameLocked end,
			function(v) PallyPower.opt.display.frameLocked = v end)
		Check("PallyPowerSimpleOptSolo", "Show the buff bar when solo", -64,
			function() return PallyPower.opt.ShowWhenSolo end,
			function(v)
				PallyPower.opt.ShowWhenSolo = v
				PallyPower:UpdateLayout()
			end)
		Check("PallyPowerSimpleOptFlyoutLeft", "Flyout opens to the left", -92,
			function() return PallyPower.opt.layout == "Layout 2" end,
			function(v)
				PallyPower.opt.layout = v and "Layout 2" or "Layout 1"
				PallyPower:UpdateLayout()
			end)

		local slider = CreateFrame("Slider", "PallyPowerSimpleOptScale", f, "OptionsSliderTemplate")
		slider:SetWidth(220)
		slider:SetPoint("TOPLEFT", f, "TOPLEFT", 25, -150)
		slider:SetMinMaxValues(0.5, 1.5)
		slider:SetValueStep(0.05)
		slider:SetObeyStepOnDrag(true)
		_G["PallyPowerSimpleOptScaleLow"]:SetText("0.5")
		_G["PallyPowerSimpleOptScaleHigh"]:SetText("1.5")
		slider:SetScript("OnShow", function(s)
			s.loading = true
			s:SetValue(PallyPower.opt.buffscale or 0.9)
			s.loading = nil
			_G["PallyPowerSimpleOptScaleText"]:SetText(format("Window scale: %.2f", s:GetValue()))
		end)
		slider:SetScript("OnValueChanged", function(s, v)
			_G["PallyPowerSimpleOptScaleText"]:SetText(format("Window scale: %.2f", v))
			if s.loading then return end
			PallyPower.opt.buffscale = v
			PallyPower.opt.configscale = v
			PallyPower:UpdateLayout()
			PallyPowerBlessingsFrame:SetScale(v)
		end)

		self.SimpleOptions = f
	end
	if self.SimpleOptions:IsShown() then
		self.SimpleOptions:Hide()
		PlaySound(SOUNDKIT.IG_SPELLBOOK_CLOSE)
	else
		self.SimpleOptions:Show()
		...
		for i = 1, PALLYPOWER_MAXCLASSES do
			...
			for j = 1, PALLYPOWER_MAXPERCLASS do
				...
				if classes[i] and classes[i][j] then
					...
					local pbIcon = _G[pbnt .. "Icon"]
					if normal ~= greater then
						-- own personal assignment: full brightness
						pbIcon:SetTexture(PallyPower.NormalBlessingIcons[normal])
						pbIcon:SetVertexColor(1, 1, 1, 1)
					else
						-- Kronos: other paladins' personal assignments show
						-- dimmed - but only for the raid lead/assist, so a
						...
						if otherBless then
							pbIcon:SetTexture(PallyPower.NormalBlessingIcons[otherBless])
							pbIcon:SetVertexColor(0.45, 0.45, 0.45, 0.9)
						else
							pbIcon:SetTexture("")
							pbIcon:SetVertexColor(1, 1, 1, 1)
						end
		...
		for i, name in pairs(SyncList) do
			...
			for id = 1, 6 do
				if SkillInfo[id] then
					...
					_G[fname .. "Icon" .. id]:SetVertexColor(1, 1, 1, 1)
					...
					-- Kronos: dim unknown blessings instead of hiding them,
					-- like the 112 build (makes gaps like untrained Kings obvious)
					_G[fname .. "Icon" .. id]:Show()
					_G[fname .. "Icon" .. id]:SetVertexColor(0.4, 0.4, 0.4, 0.4)
					_G[fname .. "Skill" .. id]:SetText("")
				end
			end
			...
			for id = 1, 3 do
				if AuraInfo[id] then
					...
					_G[fname .. "AIcon" .. id]:SetVertexColor(1, 1, 1, 1)
					...
					-- Kronos: dim unknown auras instead of hiding, like 112
					_G[fname .. "AIcon" .. id]:Show()
					_G[fname .. "AIcon" .. id]:SetVertexColor(0.4, 0.4, 0.4, 0.4)
			...
			for id = 1, 2 do
				if CooldownInfo[id] then
					...
					_G[fname .. "CSkill" .. id]:SetText(CooldownInfo[id].text or "")
				else
	...
	if isPally then
		...
		for cd, spells in pairs(self.Cooldowns) do
			for _, spell in pairs(spells) do
				...
				if not known then
					-- Kronos: IsSpellKnown can miss spellbook entries on the
					-- proxied client; a name lookup only succeeds for spells
					-- the player actually knows
					local sname = GetSpellInfo(spell)
					known = sname and GetSpellInfo(sname) ~= nil
				end
				if known then
					CooldownInfo[cd] = {}
				end
...
function PallyPower:ScanCooldowns()
	...
	local CooldownInfo = AllPallys[self.player].CooldownInfo
	for cd, spells in pairs(self.Cooldowns) do
		-- Kronos fix: the stock loop overwrote the real cooldown with the
		-- zeros returned for unknown ranks (era has no LoH rank 27154), so
		-- LoH always read "Ready". Only a real cooldown (> GCD) counts, and
		-- one missing entry must not abort the other (no early return).
		if CooldownInfo[cd] then
			local bestStart, bestDuration = 0, 0
			for _, spell in pairs(spells) do
				local start, duration = GetSpellCooldown(spell)
				if start and start > 0 and duration and duration > 1.5 then
					bestStart, bestDuration = start, duration
				end
			end
			CooldownInfo[cd].start = bestStart
			CooldownInfo[cd].duration = bestDuration
			CooldownInfo[cd].remaining = bestStart + bestDuration - GetTime()
...
-- Kronos: broadcast the status combo when a tracked cooldown starts/ends so
-- other paladins see LoH/DI availability without waiting for a full resync
local lastCdState1, lastCdState2 = false, false
function PallyPower:SendCooldownStatus()
	if GetNumGroupMembers() == 0 or not AllPallys[self.player] then
		return
	end
	local CooldownInfo = AllPallys[self.player].CooldownInfo or {}
	local s = ""
	for i = 1, 2 do
		local cdi = CooldownInfo[i]
		if cdi and cdi.duration and cdi.duration > 0 then
			s = s .. ":" .. math.floor(cdi.duration) .. ":" .. math.floor(cdi.remaining or 0)
		elseif cdi then
			s = s .. ":3600:0"
		else
			s = s .. ":n:n"
	...
	if self.opt.freeassign then
		self:SendMessage("FREEASSIGN YES SYMCOUNT " .. PP_Symbols .. " COOLDOWNS" .. s)
	else
		self:SendMessage("FREEASSIGN NO SYMCOUNT " .. PP_Symbols .. " COOLDOWNS" .. s)
	end
end

function PallyPower:SPELL_UPDATE_COOLDOWN()
	if not initialized or not isPally then
		return
	end
	self:ScanCooldowns()
	local CooldownInfo = AllPallys[self.player].CooldownInfo or {}
	local s1 = (CooldownInfo[1] and CooldownInfo[1].remaining or 0) > 1
	local s2 = (CooldownInfo[2] and CooldownInfo[2].remaining or 0) > 1
	if s1 ~= lastCdState1 or s2 ~= lastCdState2 then
		lastCdState1, lastCdState2 = s1, s2
		self:SendCooldownStatus()
	end
end
...
function PallyPower:SendSelf(sender)
	...
	-- Kronos: the proxy's old-protocol translation drops the pet class from
	-- SELF (and receivers wipe the table on SELF), so re-send the pet
	-- assignment over the bypass prefix right after
	if BuffInfo[9] and BuffInfo[9] > 0 then
		self:SendMessage("ASSIGN " .. self.player .. " 9 " .. BuffInfo[9])
	end
	s = ""
...
-- Kronos: on the wire "none" is -1. The proxy maps modern 1..6 to the old ids 0..5 and passes
-- anything else through, so a modern 0 would reach 1.12 clients as old id 0 = Wisdom.
function PallyPower:WireSkill(v)
...
function PallyPower:SendMessage(msg, type, target)
	if GetNumGroupMembers() > 0 then
		if lastMsg ~= msg then
			...
			-- Kronos interop: addon WHISPERs did not exist in the 1.12 network protocol,
			-- so a whispered reply never reaches a raid member on a vanilla client.
			-- Always broadcast to the group instead so 1.12-client paladins stay in sync.
			-- NASSIGN and pet-class ASSIGNs have no old-protocol equivalent, so
			-- they travel on the second prefix that the Kronos proxy's PLPWR
			-- translator does not touch.
			local prefix = self.commPrefix
			if strfind(msg, "^NASSIGN") or strfind(msg, "^ASSIGN %S+ 9 ") or strfind(msg, "^ASELF") or strfind(msg, "^FREEASSIGN") then
				prefix = self.commPrefix2
			end
			ChatThrottleLib:SendAddonMessage("NORMAL", prefix, msg, type)
		end
	end
end
...
function PallyPower:CHAT_MSG_ADDON(event, prefix, message, distribution, source)
	...
	if (prefix == self.commPrefix or prefix == self.commPrefix2) and (distribution == "PARTY" or distribution == "RAID" or distribution == "INSTANCE_CHAT" or distribution == "WHISPER") and sender then
...
function PallyPower:KronosStampGreater(spellID, targetName)
	...
	for i = 1, PALLYPOWER_MAXCLASSES do
		if classes[i] then
			for _, unit in pairs(classes[i]) do
				if unit.name == targetName or Ambiguate(unit.name, "none") == targetName then
					tclass, tunit = i, unit
					break
				end
			end
	...
	local function stampUnit(unit)
		...
		if unit.unitid and unit.name and IsSpellInRange(gspell, unit.unitid) == 1 then
			...
			while bn do
				if bn == gspell then
					KronosCastStamp[unit.name] = KronosCastStamp[unit.name] or {}
					KronosCastStamp[unit.name][gspell] = expire
					break
				end
				j = j + 1
				bn = UnitBuff(unit.unitid, j)
			end
		end
	end
...
function PallyPower:UNIT_SPELLCAST_SENT(event, unit, target, castGUID, spellID)
	if unit == "player" and target and target ~= "" then
		self.KronosSent = self.KronosSent or {}
		-- prune stale entries (casts that were sent but never succeeded, e.g.
		-- out of range / interrupted) so the table can't grow unbounded
		local now = GetTime()
		for guid, rec in pairs(self.KronosSent) do
			if now - rec.at > 5 then
				self.KronosSent[guid] = nil
			end
		end
		self.KronosSent[castGUID or spellID] = {target = target, at = now}
	end
end
...
function PallyPower:ParseMessage(sender, msg)
	...
	-- Kronos: the two prefixes are separate throttle queues, so status can land before the SELF that creates the entry
	if not AllPallys[sender] and (strfind(msg, "^FREEASSIGN") or strfind(msg, "^ASELF") or strfind(msg, "^SYMCOUNT")) then
		kronosPendingStatus[sender] = kronosPendingStatus[sender] or {}
		table.insert(kronosPendingStatus[sender], msg)
		return
	end
	...
	if strfind(msg, "^SELF") then
		...
		if pend then
			kronosPendingStatus[sender] = nil
			for _, m in ipairs(pend) do self:ParseMessage(sender, m) end
		end
	end
	...
	if strfind(msg, "COOLDOWNS") then
		...
		if AllPallys[sender] then
			...
			-- Kronos fix: always update (stock code only created the entry
			-- once and never refreshed it, so used cooldowns stayed "Ready")
			if remaining1 ~= "n" then
				AllPallys[sender].CooldownInfo[1] = AllPallys[sender].CooldownInfo[1] or {}
			...
			if remaining2 ~= "n" then
				AllPallys[sender].CooldownInfo[2] = AllPallys[sender].CooldownInfo[2] or {}
...
-- Kronos: last-known buff expiry by player name, so hidden units (stealthed
-- rogues) keep their buffed status instead of flashing "unbuffed"
local KronosBuffCache = {}
...
function PallyPower:ScanClass(classID)
	for _, unit in pairs(classes[classID]) do
		if unit.unitid then
			...
			local liveExpire, _, liveName = self:IsBuffActive(spell, spell2, unit.unitid)
			if liveName then
				-- the buff is visibly present on the unit
				if liveExpire and liveExpire > 0 then
					unit.hasbuff = liveExpire
					KronosBuffCache[unit.name] = GetTime() + liveExpire
				else
					-- present, but LCD has no usable timing (stealth freezes
					-- its updates and stale records go negative): keep the
					-- cached countdown ticking instead of showing blank
					local expire = KronosBuffCache[unit.name]
					local left = expire and (expire - GetTime())
					if left and left > 0 then
						unit.hasbuff = left
					else
						unit.hasbuff = 0 -- buffed, duration unknown
					end
				end
			elseif UnitBuff(unit.unitid, 1) == nil then
				-- ZERO auras on a group member = stealth/prowl blackout
				-- (rogues, druids, pets - sometimes while still visible):
				-- use the cached expiry instead of reporting "no buff"
				local expire = KronosBuffCache[unit.name]
				local left = expire and (expire - GetTime())
				if left and left > 0 then
					unit.hasbuff = left
				else
					KronosBuffCache[unit.name] = nil
					unit.hasbuff = nil
				end
			else
				-- unit shows other auras but not this blessing: genuinely unbuffed
				KronosBuffCache[unit.name] = nil
				unit.hasbuff = nil
			end
...
function PallyPower:CreateLayout()
	...
	-- Kronos: title strip in place of the drag-handle/auto/RF/aura bloat,
	-- like the 112 build. Shows the Symbol of Kings count; drag moves the
	-- bar, left-click toggles the lock, right-click opens the assignments.
	local titleBar = CreateFrame("Button", "PallyPowerTitleBar", self.Header, "BackdropTemplate")
	titleBar:SetHeight(20)
	titleBar:SetWidth(self.opt.display.buttonWidth)
	...
	titleBar:RegisterForClicks("LeftButtonUp", "RightButtonUp")
	titleBar.text = titleBar:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
	titleBar.text:SetPoint("CENTER", titleBar, "CENTER", 0, 0)
	titleBar.text:SetText("Pally Buffs")
	titleBar:SetScript("OnDragStart", function() PallyPower:DragStart() end)
	titleBar:SetScript("OnDragStop", function() PallyPower:DragStop() end)
	titleBar:SetScript("OnClick", function(_, mousebutton)
		if mousebutton == "RightButton" then
			PallyPowerBlessings_Toggle()
		else
			PallyPower.opt.display.frameLocked = not PallyPower.opt.display.frameLocked
		end
	end)
	self.titleBar = titleBar
	-- Kronos: plain black background for the assignments window, exactly like
	-- the 112 build (replaces the party-frame texture entirely)
	PallyPowerBlessingsFrame:SetBackdrop({
		bgFile = "Interface\\Buttons\\WHITE8X8",
		edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
		tile = false, edgeSize = 16,
		insets = {left = 5, right = 5, top = 5, bottom = 5},
	})
	PallyPowerBlessingsFrame:SetBackdropColor(0, 0, 0, 0.95)
	PallyPowerBlessingsFrame:SetBackdropBorderColor(1, 1, 1, 1)
	-- Kronos: drop the Blessings Report / Auto-Assign buttons and close the
	-- gap by re-anchoring Options directly to Clear
	PallyPowerBlessingsFrameReport:Hide()
	PallyPowerBlessingsFrameAutoAssign:Hide()
	PallyPowerBlessingsFrameOptions:ClearAllPoints()
	PallyPowerBlessingsFrameOptions:SetPoint("BOTTOMRIGHT", PallyPowerBlessingsFrameClear, "BOTTOMLEFT", -7, 0)
	-- Kronos: no aura column - hide it, move the class columns into its place,
	-- and shrink the window by one column width
	PallyPowerBlessingsFrameAuraGroup1:Hide()
	PallyPowerBlessingsFrameClassGroup1:ClearAllPoints()
	PallyPowerBlessingsFrameClassGroup1:SetPoint("TOPLEFT", PallyPowerBlessingsFrame, "TOPLEFT", 146, -25)
	for i = 1, PALLYPOWER_MAXPERCLASS do
		local row = "PallyPowerBlessingsFramePlayer" .. i
		_G[row .. "Aura1"]:Hide()
		_G[row .. "Class1"]:ClearAllPoints()
		_G[row .. "Class1"]:SetPoint("TOPLEFT", _G[row], "TOPLEFT", 140, 0)
		-- the row separator is hard-sized at 995 and pokes out of the
		-- narrowed window; trim it to the new inner width
		_G[row .. "Line1"]:SetWidth(908)
	end
	PallyPowerBlessingsFrame:SetWidth(1010 - 86)
	-- Kronos: rebrand the title, drop the credits tooltip, and shrink the
	-- title button so it no longer overlaps (and eats clicks meant for) the
	-- close X after the width reduction above
	PallyPowerBlessingsFrameTitle:SetWidth(840)
	PallyPowerBlessingsFrameTitle:SetScript("OnEnter", nil)
	PallyPowerBlessingsFrameTitleText:SetText("PallyPower Kronos by Mirasu")
	self:UpdateLayout()
end
...
function PallyPower:UpdateLayout()
	...
	-- Kronos de-bloat: the title strip replaces the drag handle and the
	-- auto/RF/aura buttons, so those features stay off (matches the 112 bar).
	self.opt.autobuff.autobutton = false
	self.opt.rfbuff = false
	self.opt.auras = false
	self.opt.display.enableDragHandle = false
	...
	if self.titleBar then
		self.titleBar:SetWidth(x)
		self.titleBar:ClearAllPoints()
		self.titleBar:SetPoint("BOTTOMLEFT", self.classButtons[1], "TOPLEFT", 0, 0)
		if isPally and self.opt.enabled and ((GetNumGroupMembers() == 0 and self.opt.ShowWhenSolo) or (GetNumGroupMembers() > 0 and self.opt.ShowInParty)) then
			self.titleBar:Show()
		else
			self.titleBar:Hide()
		end
		...
		if (classlist[classIndex] and classlist[classIndex] ~= 0 and (gspellID ~= 0 or self:NormalBlessingCount(classIndex) > 0)) then
			...
			for pbNum = 1, math.min(classlist[classIndex], PALLYPOWER_MAXPERCLASS) do
				...
				if IsInRaid() and gspellID == 4 and (classIndex == 1 or classIndex == 4 or classIndex == 5) and not self.opt.SalvInCombat then
					...
					-- Kronos: fall back to the normal blessing when no greater
					-- can land on this target (low level / unlearned rank)
					pButton:SetAttribute("spell1", gSpell or nSpell)
				end
		...
		if unit.unitid then
			...
			while buffName do
				if (buffName == gspell) then
					...
					-- Kronos: prefer our cast stamp when newer (greater
					-- refreshes are invisible to LCD); only in-range units are
					-- stamped, so out-of-LoS members keep their real timer
					local stampLeft = self:KronosStampLeft(unit.name, gspell)
			...
			if not found and unit.hasbuff and unit.hasbuff > 0 then
				-- Kronos: stealthed/blacked-out unit exposes no auras but the
				-- scan cache still tracks its countdown - feed that into the
				-- class button so the panel timer survives stealth too
				if unit.specialbuff then
					specialExpire = min(specialExpire, unit.hasbuff)
					specialDuration = min(specialDuration, PALLYPOWER_NORMALBLESSINGDURATION)
				else
					classExpire = min(classExpire, unit.hasbuff)
					classDuration = min(classDuration, PALLYPOWER_GREATERBLESSINGDURATION)
				end
			end
		end
	end
...
function PallyPower:UpdatePButtonOnPostClick(button, mousebutton)
	...
	if classID and playerID and classID > 0 and playerID > 0 then
...
function PallyPower:PButtonPreClick(button, mousebutton)
	...
	local playerID = button:GetAttribute("playerID")
	if classID and playerID and classID > 0 and playerID > 0 then
		local unit = classes[classID] and classes[classID][playerID]
...
function PallyPower:UpdatePButton(button, baseName, classID, playerID, mousebutton)
	...
	if unit then
		...
		if spellID ~= gspellID then
			-- personal (normal) assignment: show the normal blessing icon
			buffIcon:SetTexture(self.NormalBlessingIcons[spellID])
		else
		...
		if not InCombatLockdown() then
			...
			if IsInRaid() and (spellID == 4 or gspellID == 4) and not self.opt.SalvInCombat then
				...
				-- Greater Blessings [enabled]; Kronos: fall back to the normal
				-- blessing when no greater can land on this target (low level)
				button:SetAttribute("spell1", gSpell or nSpell)
...
function PallyPower:ButtonsUpdate()
	...
	if self.titleBar then
		self.titleBar.text:SetText("Pally Buffs (" .. (PP_Symbols or 0) .. ")")
	end
...
function PallyPower:GetUnitAndSpellSmart(classid, mousebutton)
	...
	-- Kronos: a manual class-button click must never be a dead click. When the
	-- refresh rules filter everyone out (fresh blessings everywhere), fall back
	-- to any live in-range class member so the player can always overwrite by
	-- hand. Only ButtonPreClick calls this, so auto-buff logic is unaffected.
	for _, unit in pairs(class) do
		local spellID, gspellID = self:GetSpellID(classid, unit.name)
		local nSpell, gSpell = self:CanBuffBlessing(spellID, gspellID, unit.unitid)
		local rangeSpell = gSpell or nSpell
		if rangeSpell and rangeSpell ~= "" and (IsSpellInRange(rangeSpell, unit.unitid) == 1) and (not UnitIsDeadOrGhost(unit.unitid)) then
			return unit.unitid, nSpell, gSpell
		end
	end
	return nil, "", ""
...
function PallyPower:ButtonPreClick(button, mousebutton)
	...
	-- Kronos: remember where the rotation got to; PostClick restores it when the target list is unchanged
	button.kronosStep = button:GetAttribute("step1")
	...
	if classid and classid > 0 then
		if IsInRaid() and (mousebutton == "LeftButton") and (classid ~= 10) then
			...
			-- Kronos: keep the normal blessing as a left-click fallback when
			-- no greater can land (low-level target, unlearned greater rank)
		...
		elseif not IsInRaid() or ((IsInRaid() and mousebutton == "RightButton")) then
			...
			if mousebutton == "LeftButton" then
				-- Kronos: keep the normal blessing as a left-click fallback
				-- when no greater can land (low-level target, unlearned rank)
				if gspell and gspell ~= "" then
					spell = false
				end
			end
		...
		if unitName then
			...
			elseif spell and spell ~= "" and mousebutton == "LeftButton" then
				-- Kronos: no castable greater for this target - left click
				-- falls back to the normal blessing instead of doing nothing
...
function PallyPower:ButtonPostClick(button, mousebutton)
	...
	if IsInRaid() then
		-- Create a list of viable players for the in-combat rotation script
		local targetNames = {}
		local gSpell = false
		local numPlayers = 0
		local classid = button:GetAttribute("classID")
		...
		if gSpell and numPlayers > 0 then
			-- Kronos: only rebuild the rotation when the target list or spell
			-- actually changed. The stock code rebuilt AND reset step1=1 on
			-- every click, so repeated clicks always re-cast the FIRST member
			-- instead of advancing - the "have to buff twice" bug. Preserving
			-- step across unchanged rebuilds lets clicks rotate the class.
			local sig = gSpell .. "|" .. table.concat(targetNames, ",")
			-- PreClick unwrapped the script, so always rebuild; only the step carries over
			local step = 1
			if button.kronosRotSig == sig and button.kronosStep then step = button.kronosStep end
			do
				button.kronosRotSig = sig
				button:SetAttribute("macrotext1", nil)
				button:SetAttribute("spellName1", gSpell)
				button:SetAttribute("step1", step)
				button:UnwrapScript(button, "OnClick")
				...
				button:WrapScript(button, "OnClick", [=[
					...
					if unitNames[step] and SecureCmdOptionParse("[@" .. unitNames[step] .. ",help,nodead]") then
						...
						self:SetAttribute("macrotext1", format("/cast %s %s", targetName, spellName))
					end
					self:SetAttribute("step1", step + 1)
				]=])
			end
		else
			button.kronosRotSig = nil
			button:SetAttribute("macrotext1", nil)
			button:SetAttribute("spellName1", nil)
			button:SetAttribute("step1", nil)
			button:UnwrapScript(button, "OnClick")
